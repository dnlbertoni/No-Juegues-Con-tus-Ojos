  ...
  function ordenEntregaLente($id){
    ...
    */
    ...
    while($cant<$copias){
      ...
      $texto = sprintf("Escuela: ( %s ) %s", $caso->escuela_id, $caso->escuela);
      $this->fpdf->Cell(0,5,$texto,0,1,'L');
      $texto = sprintf("Grado: %s ", $caso->grado);
      $this->fpdf->Cell(0,5,$texto,0,1,'L');
       ...
       * 
       */
      $texto = sprintf("Dia Diagnostico: %s - %s ", $caso->fechadiag, $caso->hora);
      $this->fpdf->Cell(0,5,$texto,0,1,'L');
  ...
  function cartaLentes(){
    ...
    */
    ...
    if(file_exists($nombreFile)){
      ...
      redirect('admin/cartaLentes','location',301);
    };
    ...
    /*
    foreach($alumnos as $alu){
        if($alu->escuela_id!=$escAux){
          $this->fpdf->AddPage();
          $this->fpdf->Ln(3);
	      $this->fpdf->SetFont('Arial','',16);        
          $this->fpdf->Cell(95,7,sprintf('Escuela: %s - %s ',$alu->escuela_num, $alu->escuela_nom),0,1,'L');      
          $this->fpdf->Cell(95,7,sprintf('Direccion: %s - %s ',$alu->escuela_dir, $alu->escuela_ciu),0,1,'L'); 
          $this->fpdf->Cell(95,7,sprintf('Cantidad de Alumnos: %s ',$alu->cant_prob),0,1,'L');      
          $this->fpdf->Cell(95,7,sprintf('Alumnos citados en: %s',($pes->transporte==0)?'LA FACULTAD':'LA ESCUELA'),0,1,'L');                 
          $escAux=$pes->escuela_id;
          $fil=0;
          $col=0;
    ...
    $copia=2;
       ...
       if($copia==2){
	      ...
	      //imprimo encabezado   sin Prosane
	      $this->_encabezado(TRUE, TRUE, $col,$fil);
	      //Imprimo Cuerpo
	      $this->fpdf->SetFont('Arial','',12);
	      $this->fpdf->Cell(190,7,$fecha,0,1,'R');
	      ...
	      $this->fpdf->Ln(7);
	      $this->fpdf->SetFont('Arial','',10);
	      $this->fpdf->Cell(75,4,utf8_decode(sprintf('Escuela: %s - %s ',$alu->escuela_num, $alu->escuela_nom)),0,0,'L');      
	      $this->fpdf->Cell(75,4,utf8_decode(sprintf('Direccion: %s - %s ',$alu->escuela_dir, $alu->escuela_ciu)),0,1,'L');      
	      $this->fpdf->Cell(50,4,sprintf('Grado: %s',$alu->grado),0,0,'L');      
	      $this->fpdf->Cell(50,4,sprintf('Division: %s',$alu->division),0,0,'L');      
	      $this->fpdf->Cell(50,4,sprintf('Turno: %s',$alu->turno),0,1,'L');      
	      $this->fpdf->Cell(210,4,  utf8_decode(sprintf('Apellido y Nombre: %s, %s',$alu->apellido,$alu->nombre)),0,1,'L');      
	      $this->fpdf->Cell(100,4,sprintf('D.N.I: %s',$alu->dni),0,1,'L');      
	      $yAux=$this->fpdf->GetY();
          $this->fpdf->EAN13(0+10,$yAux+3,$alu->id);
	      $this->fpdf->Cell(150,7,'',0,1,'L');      
	      $this->fpdf->Ln(15);
          $xAux=$this->fpdf->GetX();
	      $yAux=$this->fpdf->GetY();
          if($copia==1){
	      ...
	      $this->fpdf->Cell(50,7,"Autorizo",'T',1,'C');
          $this->Casos_model->mandeCartaLente($alu->id);
   	  }
    $this->fpdf->Output('cartalentes.pdf', 'D');
  }
  function turnos(){
    /**
    * imprime turnos

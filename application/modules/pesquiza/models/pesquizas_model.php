...
class Pesquizas_model extends MY_Model{
  ...
  function getAgrupadosVoluntarios(){
    $consulta = " SELECT  IF(p.voluntario_id IS NULL, 'SinAsignar', CONCAT(voluntarios.apellido, ' ' , voluntarios.nombre)) as voluntario, 
                          COUNT(p.escuela_id)
                  FROM (  SELECT DISTINCT escuela_id, voluntario_id 
                          FROM pesquizas 
                          WHERE `pesquizas`.`programa_id` = ".$this->session->userdata('programa_id').") as p 
                  INNER JOIN escuelas ON escuelas.id=p.escuela_id
                  LEFT JOIN voluntarios ON voluntarios.id=voluntario_id
